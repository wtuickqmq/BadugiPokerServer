...
JAVA_OPTIONS=" -server"
#指定进程运行的内存大小
JAVA_OPTIONS=$JAVA_OPTIONS" -Xms1g -Xmx1g -Xmn512m -XX:ParallelGCThreads=4 -XX:+UseConcMarkSweepGC -XX:+UseParNewGC"
...
#指定配置文件路径
JAVA_OPTIONS=$JAVA_OPTIONS" -Dconfig.local.table=$APP_HOME/conf/proxy-1.conf"
#指定程序的TCP端口和HTTP端口
JAVA_OPTIONS=$JAVA_OPTIONS" -Dproxy.tcp.port=$TCP_PORT -Dproxy.http.port=$HTTP_PORT -Dproxy.flash.policyport=$FLASH_PORT"
#socket远程调试模式
#JAVA_OPTIONS=$JAVA_OPTIONS" -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=0"
