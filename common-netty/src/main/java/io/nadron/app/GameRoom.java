...
public interface GameRoom {
	...
	MultiSession createMultiSession(Player player);
	...
	/**
	 * Sets the id of the game room. Preferably unique.
	 * 
	 * @param gameRoomName
	 */
	...
	/**
	 * @return Returns the id of the game room
	 */
	public abstract Long getGameRoomId();
	 ...
	 * @param gameRoomId
	 */
	public abstract void setGameRoomId(Long gameRoomId);
	 ...
	 * {@link Session#onEvent(io.nadron.event.Event)} of the GameRoom's session
	 * where the actual business logic can be applied.
	 * 
	 * @param event
	 *            The event to send to room
	 */
	public abstract void send(Event event);

	/**
	 * Method used to send a broadcast message to all sessions in the group. It
	 * is the easiest way to update the state of all connected
	 * {@link DefaultPlayer} s to the same state. This method will transmit
	 * messages using the delivery guaranty provided in the {@link NetworkEvent}
	 * .
	 * 
	 * @param networkEvent
	...
	/**
	 * Method called after the http request. Can be used to http request uri
	 * parameters and cookies
	 */
	public abstract Object onHTTPRequest(Request request);

	public abstract boolean addToRoom(PlayerSession playerSession);

	int sessionSize();

	void setRoomVariables(List<RoomVariable> roomVariables);
	
	void addRoomVariables(List<RoomVariable> roomVariables);

	RoomVariable getVariable(String key);

	void setVariable(RoomVariable roomVariable);

	List<RoomVariable> getVariables();
	
	List<Object> getVariableList() ;
