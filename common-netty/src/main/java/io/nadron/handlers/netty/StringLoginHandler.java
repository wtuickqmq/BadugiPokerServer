 ...
 */
@Sharable
public class StringLoginHandler extends SimpleChannelInboundHandler<String> {
	private static final Logger LOG = LoggerFactory.getLogger(StringLoginHandler.class);
	...
	@Override
	public void channelRead0(ChannelHandlerContext ctx, String frame) throws Exception {
		Channel channel = ctx.channel();
		String data = frame;
		LOG.trace("From String: " + data);
		// if(! (frame.startsWith("{")) ){
		// closeChannelWithLoginFailure(channel);
		// return ;
		// }
		if (frame.trim().length() < 2 && !(frame.startsWith("{") && frame.endsWith("}")))
			return;
		...
		if (Events.LOG_IN == type) {
			...
			if (null != credList && credList.size() > 2) {
	...
	protected void handleReJoin(PlayerSession playerSession, GameRoom gameRoom, Channel channel) {
		...
		channel.writeAndFlush(Events.GAME_ROOM_JOIN_SUCCESS, null);// assumes
																	// that the
																	// protocol
																	// applied
																	// will take
	...
	public void handleGameRoomJoin(Player player, Channel channel, String refKey) throws Exception {
		...
		if (null != gameRoom) {
			...
			LOG.trace("Sending GAME_ROOM_JOIN_SUCCESS to channel {}", channel);
			 channel.writeAndFlush(eventToFrame(Events.GAME_ROOM_JOIN_SUCCESS, reconnectKey));
	...
	public void connectToGameRoom(final GameRoom gameRoom, final PlayerSession playerSession, Channel channel) {
		...
		LOG.debug("send login start....");
		gameRoom.onLogin(playerSession);
		
		tcpSender.sendMessage(Events.event(null, Events.START));
	}

	protected String eventToFrame(byte opcode, Object payload) throws Exception {
		Event event = Events.event(payload, opcode);
		String value = jackson.writeValueAsString(event);
		return value;
	}
