 ...
 * {@link Session#onEvent(Event)}. The event dispatcher will route all events to
 * this class's {@link #onEvent(Event)} method. It provides default
...
public class DefaultSessionEventHandler implements SessionEventHandler {
	...
	protected void onReconnect(ConnectEvent event) {
		Session session = getSession();
		// To synchronize with task for closing session in ReconnectRegistry
		// service.
	...
	protected void onException(Event event) {
		...
		session.setUDPEnabled(false);// will be set to true by udpupstream
										// handler on connect event.
	...
	private void logNullTcpConnection(Event event) {
		LOG.warn("Discarding {} as TCP connection is not fully " + "established for this {}", event, getSession());
	}
}
