...
public class TCPPipelineFactory extends ChannelInitializer<SocketChannel> {
	 ...
	 * nadron server.
	 */
	protected static final LengthFieldPrepender LENGTH_FIELD_PREPENDER = new LengthFieldPrepender(2);
	/**
	 * Decodes incoming messages from remote nadron server to
	 * {@link MessageBuffer} type, puts this as the payload for an {@link Event}
	 * and passes this {@link Event} instance to the next decoder/handler in the
	 * chain.
	 */
	protected static final MessageBufferEventEncoder EVENT_ENCODER = new MessageBufferEventEncoder();
	/**
...
//		stringPieline(pipeline);
		messageBufferpipeline(pipeline);
	}

	public void objectPipeline(ChannelPipeline pipeline) {
		...
		pipeline.addLast(DefaultToClientHandler.getName(), defaultToClientHandler);
		pipeline.addLast("lengthFieldPrepender", new LengthFieldPrepender(2));
		pipeline.addLast("eventEncoder", new EventObjectEncoder());
	}

	public void stringPieline(ChannelPipeline pipeline) {
		pipeline.addLast("decode", new StringDecoder());
		pipeline.addLast(DefaultToClientHandler.getName(), defaultToClientHandler);
		pipeline.addLast("encode", new StringEncoder());
	}

	public void messageBufferpipeline(ChannelPipeline pipeline) {
		pipeline.addLast("lengthDecoder", new LengthFieldBasedFrameDecoder(Integer.MAX_VALUE, 0, 2, 0, 2));
