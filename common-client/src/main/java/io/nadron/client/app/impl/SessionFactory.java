 ...
 * the actual connection to the nadron server by initializing
 * {@link NettyTCPClient} and {@link NettyUDPClient} and using their connect
 * methods.
 * 
...
public class SessionFactory {
	 ...
	 * @throws Exception
	 */
	public SessionFactory(final LoginHelper theLoginHelper) throws UnknownHostException, Exception {
	 ...
	 * 
	 * @return The session instance created and connected to remote nadron
	 *         server.
	 ...
	 * @throws Exception
	 */
	public Session createAndConnectSession(EventHandler... eventHandlers) throws InterruptedException, Exception {
	...
	public Session createSession() {
		SessionBuilder sessionBuilder = new SessionBuilder().id(sessionId.incrementAndGet());
	 ...
	 * @throws Exception
	 */
	public void connectSession(final Session session) throws InterruptedException, Exception {
	...
	public void connectSession(final Session session, EventHandler... eventHandlers) throws InterruptedException, Exception {
		...
		if(null!=loginHelper.getType()&&loginHelper.getType()==1){
			loginEvent = Events.event(buffer, Events.LOG_IN_WITH_JSON);
		}else{
			loginEvent = Events.event(buffer, Events.LOG_IN);
		}
		//
		// List<Object> list = loginHelper.getLoginList();
		// Event loginEvent = Events.event(list, Events.LOG_IN);
	...
	/**
	 * Set the channel initializer. This will be used when connecting the
	 * session.
	 * 
	 * @param channelInitializer
	 */
	protected synchronized void setTCPChannelInitializer(ChannelInitializer<SocketChannel> channelInitializer) {
