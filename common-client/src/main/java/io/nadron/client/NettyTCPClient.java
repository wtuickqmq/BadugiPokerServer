 ...
 * nadron server. Using one instance of this class, multiple tcp connections can
 * be made to a remote nadron server. For connection to multiple nadron server's
...
public class NettyTCPClient {
	 ...
	 */
	public static final ChannelGroup ALL_CHANNELS = new DefaultChannelGroup("NAD-CLIENT-CONNECTIONS", GlobalEventExecutor.INSTANCE);
	...
	public NettyTCPClient(final InetSocketAddress serverAddress) {
		this(serverAddress, new NioEventLoopGroup(Runtime.getRuntime().availableProcessors() * 2 + 1), 5000);
	}
	 ...
	 */
	public NettyTCPClient(final InetSocketAddress serverAddress, final EventLoopGroup boss, final int maxShutdownWaitTime) {
	 ...
	 * @param loginEvent
	 *            The event contains the {@link ByteBuf} to be transmitted to
	 *            nadron server for logging in. Values inside this buffer
	 *            include username, password, connection key, <b>optional</b>
	 *            local address of the UDP channel used by this session.
	 * @return Returns the Netty {@link Channel} which is the connection to the
	 *         remote nadron server.
	 * @throws InterruptedException
	 */
	public Channel connect(final ChannelInitializer<SocketChannel> pipelineFactory, final Event loginEvent) throws InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
	public Channel connect(final ChannelInitializer<SocketChannel> pipelineFactory, final Event loginEvent, int timeout, TimeUnit unit) throws InterruptedException {
		...
		synchronized (bootstrap) {
			...
			future.addListener(new ChannelFutureListener() {
				...
				public void operationComplete(ChannelFuture future) throws Exception {
					if (future.isSuccess()) {
						// String value =
						// BeanUtil.JACKSON.writeValueAsString(loginEvent);
						// future.channel().writeAndFlush(value);
