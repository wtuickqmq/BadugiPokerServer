/**
 * 深圳市辰富科技有限公司 .inc
 * Texas Poker Game App V1.0
 */
/**
 * author  yuesheng.yin
 * createTime 2011-1-21下午03:59:00
 */
package com.badugi.poker.pokerhand.handtype;
import java.util.List;

import com.badugi.poker.Card;
import com.badugi.poker.TexasConstant.PokerHandType;
import com.badugi.poker.pokerhand.PokerHandBase;


/**
 * 同花顺(同一花色的顺子)
 * 
 * @author yuesheng.yin 牌序按照自然顺序存放
 */
public class StraightFlush extends PokerHandBase implements Cloneable{

	private static final long serialVersionUID = 7459510591887295575L;

	public StraightFlush(List<Card> cardLi) {
		super.setCards(cardLi.toArray(new Card[cardLi.size()]));
	}
	
	public StraightFlush() {
		
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see com.texas.logic.pokerhand.PokerHand#getPokerHandType()
	 */
	@Override
	public PokerHandType getPokerHandType() {
		// TODO Auto-generated method stub
		return PokerHandType.STRAIGHT_FLUSH;
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see java.lang.Comparable#compareTo(java.lang.Object)
	 */
	public int compareTo(PokerHandBase o) {
		if (getPokerHandType().getValue() != o.getPokerHandType().getValue())
			return getPokerHandType().getValue() - o.getPokerHandType().getValue();
//		return super.getCard(0).compareTo(o.getCard(0));// 同种类型的牌的比对规则
		return super.getCard(2).compareTo(o.getCard(2));// 同种类型的牌的比对规则
	}
	
	@Override
	protected Object clone() {
		StraightFlush flush = null;
		try {
			flush = (StraightFlush) super.clone();
		} catch (CloneNotSupportedException e) {
			e.printStackTrace();
		}
		return flush;
	}

}
